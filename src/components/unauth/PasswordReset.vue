<template>
  <v-container>
    ...
    <div style="width: 50%; margin: 0 auto;">
      <v-row class="content-wrap" align="center" justify="center">
        <v-text-field placeholder="New password" label="New password" v-model="password"
                :rules="passwordRules"
                class="input-group--focused"
        ></v-text-field>
      </v-row>
      <v-row class="content-wrap" align="center" justify="center">
        <v-text-field placeholder="Repeated password" label="Repeated password" v-model="repeatedPassword"
                :rules="repeatedPasswordRule"
                class="input-group--focused"
        ></v-text-field>
      </v-row>
      <v-row>
        <v-btn @click='reset' color="green" style="margin: 0 auto;">Reset password</v-btn>
      </v-row>
    </div>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    longerThanXRule: (x) => v => v.length > x || 'Password must be at least 8 chars long.',
